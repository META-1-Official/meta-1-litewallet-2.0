...
export const OrdersTable = (props) => {
    const { data, column, direction } = props

    useEffect(async () => {
        console.log(await Meta1.assets['1.3.0'])
        console.log(await Meta1.history.get_account_history("kj-test2", "1.11.0", 10, "1.11.0"))
    }, [])
                ...
                <Table sx={{ minWidth: 700 }} aria-label="customized table">
                    <TableHead>
                        <TableRow style={{ display: 'table-row' }}>
                            <StyledTableCell sorted={column === 'id' ? direction : null}
                                             onClick={() => {}} align="left">ASSETS</StyledTableCell>
                    ...
                    <TableBody>
                        {data.map(({ id, qty, expiration, amount }) => (
...
const PortfolioTable = (props) => {
    ...
    useEffect(() => {
        getDatas();
        data.forEach((d, i) => {
            let precision = assets.filter((asset) =>
                asset.symbol.includes(d.name)
            )
    ...
    function getDatas(){
        Meta1.ticker('USDT', 'BTC')
            .then(res => setBtc(res))
        Meta1.ticker('USDT', 'BNB')
            .then(res => setBnb(res))
        Meta1.ticker('USDT', 'LTC')
            .then(res => setLtc(res))
        Meta1.ticker('USDT', 'XLM')
            .then(res => setXlm(res))
        Meta1.ticker('USDT', 'ETH')
            .then(res => setEth(res))
        Meta1.ticker('USDT', 'EOS')
            .then(res => setEos(res))
        Meta1.ticker('USDT', 'META1')
            .then(res => setMeta1(res))
    }
    ...
    return (
        <TableContainer component={Paper} style={{borderRadius: '4px', height: '100%', boxShadow: '0 2px 10px 0 rgba(0, 0, 0, .11)'}}>
            <Table sx={{ minWidth: 700 }} aria-label="customized table" style={{height: '100%'}}>
                <TableHead>
                    <TableRow>
                        <StyledTableCell
                        ><div className="table_title">Asset</div></StyledTableCell>
                        <StyledTableCell
                    ...
                    {lists?.map((data) => (
                        <StyledTableRow key={data?.name}>
                            ...
                            <StyledTableCell align="center" className={'currencyValues'}>
                                {(currencyValue(data, meta1) * Number(localStorage.getItem('currency').split(' ')[2])).toFixed(2)}
                            </StyledTableCell>
                            <StyledTableCell align="left">{<div className={Number(cryptoAllInfo(data.name).percent_change) >= 0 ? 'plus' : 'minus'}>{cryptoAllInfo(data.name).percent_change >=0 ? '+' + cryptoAllInfo(data.name).percent_change: cryptoAllInfo(data.name).percent_change}%</div>}</StyledTableCell>
                            <StyledTableCell align="left" className={'currencyPrices'}>{(currencyPrice(data, meta1) * Number(localStorage.getItem('currency').split(' ')[2])).toFixed(3)}</StyledTableCell>
...
function Wallet(props) {
    ...
    async function doAllUsefulThings() {
        let response = await Meta1.history.get_account_history(accountName, '1.11.0', 10, '1.11.0')
        setOrders(response)
    }
    ...
    const openDrop = () => {
        document.getElementById('switchContainer').style.display === 'none' ? document.getElementById('switchContainer').style.display = 'block' : document.getElementById('switchContainer').style.display = 'none';
    }
    ...
    useEffect(() => {
        let summTik = 0, percentage = 0, totSum = 0, lastChange = 0;
        ...
        setTotalChange(Number(ch) > 0 ? ch.toFixed(2).toString() : ch.toFixed(2).toString())
    ...
    const changeCurrencyToFiat = async () => {
        ...
        document.getElementById('forCheck').innerText = localStorage.getItem('currency').split(' ')[1];
    }
    ...
    const changeCryptoCurrency = async (e) => {
        ...
        if (crypto !== 'META1') {
            if (document.getElementById('forCheck').innerText === 'META1') {
                ...
                setTimeout(() => {
                    ...
                    for (let i = 0; i<values.length; i++) {
                        values[i].innerText = Number(values[i].innerText * data[crypto]).toFixed(7).toString()
                        prices[i].innerText = Number(prices[i].innerText * data[crypto]).toFixed(7).toString()
                    }
            ...
            setTimeout(() => {
                ...
                let prices = document.getElementsByClassName('currencyPrices')
                for (let i = 0; i<values.length; i++) {
                    values[i].innerText = Number(values[i].innerText * meta1.latest).toFixed(7).toString()
                    prices[i].innerText = Number(prices[i].innerText * meta1.latest).toFixed(7).toString()
                }
                document.getElementById('forCheck').innerText = crypto
            }, 2000)
    ...
    return (
        <>
                <div style={{marginLeft: '3rem'}} className={'totalSumBlock'}>
                    <h4 style={{color: '#505361', fontSize: '.9rem', marginBottom: '.3rem'}}>Portfolio Balance</h4>
                    <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between'}}>
                        <div className={'blockSumAndPercentage'} style={{display: 'flex', flexDirection: 'row'}}>
                            <h2 style={{color: '#FFC000', fontSize: '2rem'}}><strong>{loader ? <Loader size="mini" active inline="centered" /> : localStorage.getItem('currency').split(' ')[0] + (totalSum * localStorage.getItem('currency').split(' ')[2])}</strong></h2>
                            <h5 style={{margin: '.3rem 0 .5rem 1rem', fontSize: '.8rem', height: '40%', padding: '.3rem', borderRadius: '3px', boxShadow: '0 4px 9px 5px rgba(0,0,0,.11)'}} className={Number(totalChange) > 0 ? 'plusFirst' : 'minusFirst'}>
                                {Number(totalChange) > 0 ? <i className="far fa-arrow-up fa-xs" style={{color: '#fff', marginRight: '.2rem'}}/> : <i className="far fa-arrow-down fa-xs " style={{color: '#fff', marginRight: '.2rem'}}/> }
                                {loader ? null : totalChange.replace('-', '')} %</h5>
                        </div>
                        <div className={'blockChoose'}>
                            <noscript id={'forCheck'}>{localStorage.getItem('currency').split(' ')[1]}</noscript>
                            <div className={'blockChoosen'} onClick={openDrop}>
                                <span>Select currency to display...</span>
                                <i className="fas fa-chevron-down" style={{marginTop: '.2rem'}}/>
                            </div>
                            <div id={'switchContainer'} style={{position: 'relative', display: 'none'}}>
                                <ul className={'chooseContainer'}>
                                    <li style={{padding: '.5rem'}} onClick={changeCurrencyToFiat}>
                                        <img src={fiatIcon} style={{width: '35px'}} alt="cryptoImage"/>
                                        <span style={{marginLeft: '3rem'}}>Fiat ({localStorage.getItem('currency').split(' ')[0]})</span>
                                    </li>
                                    {assets.map(el => (
                                        <li style={{padding: '.5rem'}} onClick={changeCryptoCurrency}>
                                            <img src={el.image} style={{width: '35px'}} alt="cryptoImage"/>
