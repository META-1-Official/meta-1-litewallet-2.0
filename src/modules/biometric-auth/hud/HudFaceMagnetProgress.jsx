...
const HudFaceMagnetProgress = forwardRef((props={
  _canvasWidth: 0,
  _canvasHeight: 0
}, ref) => {
  ...
  const backendWidth = 720, backendHeight = 480;
  ...
  function getFillColor(percentage) {
    ...
    const red = clampedPercentage > 50 ? 255 - ((clampedPercentage - 50) * 2 * 255 / 100) : 255;
    const green = clampedPercentage < 50 ? (clampedPercentage * 2 * 255 / 100) : 255;
  ...
  useEffect(() => {
    if (!data || !data.face_bounds) {
      ...
      if (typeof canvasWidth === "undefined" || canvasWidth < 50 || typeof canvasHeight === "undefined" || canvasHeight < 50) return
      ...
      if (data.face_liveliness) {
        ...
        if (oldTask && subPipelineNames[subPipelineNames.length - 1] !== oldTask) {
          // task changed
          // Check why ?
          if (subPipelineNames.length >= 2 && subPipelineNames[subPipelineNames.length - 2] === oldTask) {
        ...
        const percentage = (lastPipeline.current_score / lastPipeline.target_score) * 100;
