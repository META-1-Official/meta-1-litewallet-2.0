...
const parsePipelineData = (data) => {
  ...
  if (face_detection === 0) return {text: pipelineTextMap.face_detection, loading: true};
  if (face_detection === 100 && face_verification === 0) return {
  ...
  if (
    ...
    return {text: pipelineTextMap.face_liveliness[lastSubPipeline], loading: false};
  }
...
const HudUserGuidanceAlert = forwardRef((props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    updateData: (newData) => {
      ...
      if (newMessageData.text !== messageData.text || newMessageData.loading !== messageData.loading) {
